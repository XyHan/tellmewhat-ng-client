...
export class AuthService implements AuthServiceInterface {
  ...
  public async login(email: string, password: string): Promise<DecodedTokenInterface | null> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        token && token.token.length ? resolve(plainToClass(DecodedTokenModel, token.token)) : resolve(null);
      } catch (e) {
